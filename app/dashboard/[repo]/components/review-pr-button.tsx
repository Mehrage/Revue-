...
export function ReviewPrButton({ owner, repo, prNumber, alreadyReviewed }: Props) {
  ...
  return (
    ...
    <div className={review ? "w-full basis-full mt-4" : "shrink-0 ml-auto flex items-center gap-4"}>
      ...
      {/* The 3-Column Review Box (Unchanged) */}
      {review && (
        <div className="flex flex-col w-full animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex w-full min-h-[350px] gap-5 p-5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)", border: "1px solid rgba(255,255,255,0.03)" }}>
            
            <div className="flex-1 flex flex-col rounded-xl p-5 overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.018)" }}>
              <h3 style={{ color: GOLD }} className="text-[10px] font-semibold uppercase tracking-[0.1em] mb-3">AI Summary</h3>
              <div className="flex-1 overflow-y-auto text-[#c0c3d4] text-xs leading-relaxed whitespace-pre-wrap break-words pr-2 custom-scrollbar">
                {review.text}
              </div>
            </div>

            <div className="w-28 shrink-0 flex flex-col justify-center gap-2.5">
              {["Analyzing", "Logic Check", "Security", "Formatting", "Complete"].map((step, i) => (
                <div key={step} className="h-9 w-full rounded-lg flex items-center justify-center text-[11px] font-medium transition-colors"
                  style={{
            ...
            <div className="flex-1 flex flex-col rounded-xl p-5 overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.018)" }}>
               <h3 style={{ color: GOLD }} className="text-[10px] font-semibold uppercase tracking-[0.1em] mb-3">Visualization</h3>
              <div className="flex-1 flex items-center justify-center rounded-lg border border-dashed border-white/10 bg-black/20 text-xs text-[#52556a] overflow-auto">
                {review.graph ? <MermaidChart chart={review.graph} /> : "No visual architecture changes detected in this PR."}
              </div>
            </div>
          </div>

          <button onClick={() => setReview(null)} className="self-end mt-3 px-3 py-1.5 rounded-md hover:bg-white/5 transition-colors" style={{ fontSize: "11px", color: "#52556a" }}>
            Dismiss Review
          </button>
        </div>
      )}
